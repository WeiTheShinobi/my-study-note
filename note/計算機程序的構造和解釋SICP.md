...
組合不同的過程得到抽象，我們也可以以此來組合數據，像是一個座標有 x, y 軸，如果這兩個值每次都被分開，我們也難以理解和開發，組合不同的數據成為抽象，程式語言提供「膠水」來組合數據。

`point(x, y).compute()`

不必知道 `compute()`如何計算，不必知道 x, y，對我們來說這就是一個抽象。

### 抽象屏障

數據是什麼與使用數據的過程是分開的，彼此不接觸，也關係到`point`的建構。
...
本章將焦點放在程式的兩種世界觀：物件導向和資料流

---

一個物件「有狀態」，即是它的行為會受到歷史操作的影響，先前的程式碼中我們不會改變名字的值，而現在會，這改變了以前的求值模型，同樣的名字卻有不同的結果。

# 寄存器機器裡的計算

有一個數據通路和控制器，數據通路顯示一個程式執行時需要的寄存器、操作（=, +, -...），控制器可以控制數據通路上的按鈕，只要控制器按照順序，程式就能正確地執行。
